...
async def create_book(call: types.CallbackQuery, state: FSMContext):
	...
	await call.message.edit_text(text=new_book_title_text, reply_markup=await ikb.back(bot_page='start'))
...
async def new_book_title(message: types.Message, state: FSMContext):
	title = message.text
	if len(title) > 256:
		await message.answer(text=max_lenght_text, reply_markup=await ikb.back(bot_page='start'))

	else:
		await NewBook.author.set()
		await state.update_data(title=title)

		await message.answer(text=new_book_author_text, reply_markup=await ikb.back(bot_page='start'))
...
async def new_book_author(message: types.Message, state: FSMContext):
	...
	if len(author) > 256:
		...
		await message.answer(text=new_book_description_text, reply_markup=await ikb.back(bot_page='start'))
...
async def new_book_description(message: types.Message, state: FSMContext):
	...
	await message.answer(text=new_book_category_text, reply_markup=await ikb.book_categories(bot_page='start',
...
async def new_book_category_mes(message: types.Message, state: FSMContext):
	...
	if len(category_title) > 256:
		...
		await db.add_book(title=title, author=author, description=description, category=category_title)
		await message.answer(text=await new_book_added_text(title=title, category_title=category_title))
...
async def new_book_category_call(call: types.CallbackQuery, state: FSMContext):
	...
	if call.data.startswith('goto_'):
		...
		await state.finish()
		await db.add_book(title=title, author=author, description=description, category=category)
		await call.message.answer(text=await new_book_added_text(title=title, category_title=category))
