...
from .postgress_handler import postgres_do_query, postgres_select_all, postgres_select_one

class Database:

	@staticmethod
	async def create_tables():
		await postgres_do_query('''CREATE TABLE IF NOT EXISTS public.categories 
								(
								id serial,
								category_title varchar(256) PRIMARY KEY COLLATE pg_catalog."default"
								)''')

		await postgres_do_query('''CREATE TABLE IF NOT EXISTS public.books
								(
								id serial,
...
#--------Table categories---------
	...
	async def add_category(title: str):
		try:
			await postgres_do_query("INSERT INTO categories (category_title) VALUES(%s)",
			                        (title,))
	...
	async def delete_category(id: int):
		try:
			await postgres_do_query("DELETE FROM categories WHERE id = %s",
			                        (id,))
...
#-------Table books---------
	...
	async def add_book(title: str, author: str, description: str, category: str):
		try:
			await postgres_do_query("INSERT INTO books (title, author, description, category) "
			                        "VALUES(%s, %s, %s, %s)",
