...
async def start():
    ...
    create_book_button = InlineKeyboardButton(
        text="➕ Добавить книгу", callback_data="create_book"
    )
...
async def book_categories(
    ...
    pages = (len(categories) // categories_on_page) + 1
    ...
    add_category_button = InlineKeyboardButton(
        text="➕ Добавить жанр", callback_data="add_category"
    )
    ...
    page = current_page - 1
    categories_for_page = categories[
        page * categories_on_page : categories_on_page * current_page
    ]
    for category in categories_for_page:
        ikb.add(
            InlineKeyboardButton(
                text=f"{category['category_title']}",
...
# клавиатура выбора типа отображения книг
async def select_display_books_type(bot_page: str):
    ...
    categories_books_button = InlineKeyboardButton(
        text="💈 По жанрам", callback_data="books_by_categories"
    )
    back_button = InlineKeyboardButton(text="🔚 Назад", callback_data=f"back_{bot_page}")
...
# клавиатура отображения книг
async def books(bot_page: str, books: list, current_page: int):
    ikb = InlineKeyboardMarkup()
    books_on_page = 6
    pages = (
        (len(books) // books_on_page)
        if len(books) >= books_on_page
        else (len(books) // books_on_page) + 1
    )
    next_page = (current_page + 1) if current_page != pages else 1
    previous_page = (current_page - 1) if current_page != 1 else pages
...
# клавиатура внутри книги
async def book(bot_page: str, book_id: int):
    ikb = InlineKeyboardMarkup(row_width=1)

    delete_book_button = InlineKeyboardButton(
        text="🗑 Удалить", callback_data=f"delete_book_{book_id}"
    )
    back_button = InlineKeyboardButton(text="🔚 Назад", callback_data=f"back_{bot_page}")
